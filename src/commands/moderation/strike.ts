...
async function execute(interaction: ChatInputCommandInteraction, client: BotClient): Promise<void> {
  ...
  try {
    ...
    const embed = buildInfractionEmbed({
      ...
      moderatorTag: interaction.user.tag,
      type: 'Strike', reason, evidence, caseId,
    });
    ...
    await interaction.editReply({ embeds: [buildSuccessEmbed('Strike Issued', `${target.tag} has been struck. Case #${caseId}`)] });
  } catch (err) {
...
async function prefixExecute(message: Message, args: string[], client: BotClient): Promise<void> {
  await message.reply({ embeds: [buildErrorEmbed('Use Slash Command', 'Please use `/strike` for this action.')] });
}
