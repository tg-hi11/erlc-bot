...
const data = new SlashCommandBuilder()
  ...
  .addUserOption((o) => o.setName('user').setDescription('User to set rank for').setRequired(true))
...
async function execute(interaction: ChatInputCommandInteraction, client: BotClient): Promise<void> {
  ...
  try {
    ...
    await interaction.editReply({
      embeds: [buildSuccessEmbed('Rank Set', `${target.tag} rank set to **${newRole.name}**.`)],
    });
  } catch (err) {
